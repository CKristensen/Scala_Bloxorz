...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    if(!b.isLegal) LazyList()
    else b.legalNeighbors map ((x: (Block, Move)) => (x._1, List(x._2) ++ history)) to(LazyList)
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if(initial.isEmpty) LazyList()
      ...
      val sol = for {n <- initial
                    next <- newNeighborsOnly(
                      neighborsWithHistory(n._1, n._2), explored)
                     } yield next
      initial #::: from(sol, explored ++ initial.map(_._1))
    }
  }
   ...
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(LazyList((startBlock, List[Move]()))
    , Set[Block]())
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart filter ((x: (Block, List[Move])) => done(x._1))
  /**
